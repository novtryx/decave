...
export default function OrderSummary({ 
  ...
  const handleQuantityChange = (newQuantity: number) => {
    ...
    if (newQuantity > MAX_QUANTITY) return; // Fix for Issue #2: Max limit of 5
        ...
        <div className="mt-6 bg-[#0F0F0F] border border-[#2a2a2a] rounded-lg p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[#99A1AF] text-sm">Select number of tickets (max {MAX_QUANTITY})</p>
            </div>
            <div className="flex items-center gap-4">
              <button 
                onClick={() => handleQuantityChange(quantity - 1)}
                disabled={quantity <= 1 || isProcessing}
                className="w-10 h-10 rounded-full bg-[#2a2a2a] flex items-center justify-center text-white text-lg hover:bg-[#3a3a3a] disabled:opacity-30 disabled:cursor-not-allowed transition-all"
              >
                -
              </button>
              ...
              <button 
                onClick={() => handleQuantityChange(quantity + 1)}
                disabled={quantity >= MAX_QUANTITY || isProcessing}
                className="w-10 h-10 rounded-full bg-[#2a2a2a] flex items-center justify-center text-white text-lg hover:bg-[#3a3a3a] disabled:opacity-30 disabled:cursor-not-allowed transition-all"
              >
                +
              </button>
          ...
          {quantity >= MAX_QUANTITY && (
            <div className="mt-3 p-2 bg-[#3A2A1A] border border-[#CCA33A] rounded-lg">
              <p className="text-[#CCA33A] text-sm font-medium">
                ℹ️ Maximum {MAX_QUANTITY} tickets per purchase
              </p>
            </div>
          )}
        </div>
        ...
        <div className="mt-8 py-6 border-y border-[#2a2a2a] flex flex-col gap-4">
          ...
          <div className="flex justify-between">
            <div>
              ...
              <p className="text-[#666] text-sm">0% of subtotal</p>
            </div>
