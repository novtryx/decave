...
//           onViewDetails={() => router.push('/events/afrospook-2025')}
//         />
//       </section>
...
//       0
//     );
//     return soldTickets > 1000 ? `${(soldTickets / 1000).toFixed(1)}K+` : `${soldTickets}+`;
//   };
...
//           <FeaturedEventCard
//             image={featuredEvent.eventDetails.eventBanner}
...
//             onViewDetails={() => router.push(`/events/${featuredEvent.id}`)}
//           />
//         </section>
//       )}
...
import { useRouter } from "next/navigation";
import TabNavigation from "@/components/layout/TabNavigation";
import { getPublishedEvents, generateSlug, type Event } from "@/app/actions/events";
      ...
      {featuredEvent && (
        <section className="bg-[#0f0f0f] mt-10 px-6 lg:px-16 py-14">
          ...
          <FeaturedEventCard
            ...
            location={`${featuredEvent.eventDetails.venue} - ${featuredEvent.eventDetails.address}`}
            onViewDetails={() => router.push(`/events/${generateSlug(featuredEvent.eventDetails.eventTitle)}`)}
          />
        </section>
      )}
            ...
            {upcomingEvents.slice(0, 6).map((event) => {
              ...
              return (
                <ImageCard
                  ...
                  buttonText={isSoldOut ? "Sold Out" : "View Event"}
                  buttonVariant="outline"
                  onButtonClick={() => router.push(`/events/${generateSlug(event.eventDetails.eventTitle)}`)}
                />
              );
            })}
          </div>
